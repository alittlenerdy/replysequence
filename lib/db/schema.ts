...
export type BriefingStatus = 'generating' | 'ready' | 'sent' | 'failed';
export type BriefingDeliveryChannel = 'dashboard' | 'email' | 'both';
...
export interface BriefingAttendeeInsight {
  name: string;
  email?: string;
...
export const preMeetingBriefings = pgTable(
  'pre_meeting_briefings',
  {
    ...
    meetingStartTime: timestamp('meeting_start_time', { withTimezone: true }).notNull(),
    meetingPlatform: meetingPlatformEnum('meeting_platform'),
    meetingUrl: text('meeting_url'),
    // Briefing content (generated by Claude)
    content: jsonb('content').$type<BriefingContent>(),
    // Generation tracking
    status: varchar('status', { length: 20 }).$type<BriefingStatus>().notNull().default('generating'),
    ...
    errorMessage: text('error_message'),
    // Delivery tracking
    deliveryChannel: varchar('delivery_channel', { length: 20 }).$type<BriefingDeliveryChannel>().default('both'),
  ...
  (table) => [
    index('pre_meeting_briefings_user_id_idx').on(table.userId),
    index('pre_meeting_briefings_calendar_event_id_idx').on(table.calendarEventId),
    index('pre_meeting_briefings_status_idx').on(table.status),
    index('pre_meeting_briefings_meeting_start_time_idx').on(table.meetingStartTime),
    uniqueIndex('pre_meeting_briefings_event_unique_idx').on(table.userId, table.calendarEventId),
  ]
);
