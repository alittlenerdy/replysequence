...
function inlineFormat(text: string): string {
  ...
  text = text.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    '<a href="$2" class="text-[#6366F1] light:text-[#4F46E5] underline hover:text-[#818CF8] light:hover:text-[#6366F1]">$1</a>'
  );
  // Inline code: `code`
  text = text.replace(
    /`([^`]+)`/g,
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { '@type': 'ListItem', position: 2, name: 'Blog', item: 'https://www.replysequence.com/blog' },
              { '@type': 'ListItem', position: 3, name: post.title, item: `https://www.replysequence.com/blog/${post.slug}` },
            ],
          }),
        }}
      />
