...
function getTimeSinceBadge(meetingStartTime: Date | null): { label: string; className: string; title: string } | null {
  ...
  if (hours < 2) {
    return {
      label: '< 2h',
      className: 'bg-[#6366F1]/15 text-[#6366F1] border-[#6366F1]/30',
  ...
  if (hours < 24) {
    return {
      ...
      className: 'bg-[#6366F1]/15 text-[#818CF8] border-[#6366F1]/30',
...
function getQualityBadgeStyle(stars: number): { bg: string; text: string; label: string } {
  if (stars >= 4) return { bg: 'bg-[#6366F1]/20 border-[#6366F1]/40', text: 'text-[#6366F1]', label: stars === 5 ? 'Excellent' : 'Good' };
  if (stars === 3) return { bg: 'bg-[#6366F1]/20 border-[#6366F1]/40', text: 'text-[#818CF8]', label: 'Review' };
    ...
    switch (platform) {
      case 'zoom':
        return (
          <div className="w-8 h-8 relative group/icon">
            <div className="absolute inset-0 bg-[#6366F1]/30 rounded-lg blur-md group-hover/icon:bg-[#6366F1]/50 transition-colors duration-300" />
            <div className="relative p-1.5 rounded-lg bg-gradient-to-br from-[#6366F1] to-[#4F46E5] shadow-lg shadow-[#6366F1]/30">
      ...
      case 'microsoft_teams':
        return (
          <div className="w-8 h-8 relative group/icon">
            <div className="absolute inset-0 bg-[#6366F1]/30 rounded-lg blur-md group-hover/icon:bg-[#6366F1]/50 transition-colors duration-300" />
            <div className="relative p-1.5 rounded-lg bg-gradient-to-br from-[#6366F1] to-[#3A4BDD] shadow-lg shadow-[#6366F1]/30">
              <svg className="w-4 h-4 text-white" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    return (
      <span
        className={`inline-flex items-center justify-center w-5 h-5 rounded-full ${
          isUp ? 'bg-[#6366F1]/20 text-[#6366F1]' : 'bg-red-500/20 text-red-400'
        }`}
      ...
      >
        <span className="flex">
          {[1, 2, 3, 4, 5].map((s) => (
            <svg
              key={s}
              className={`${compact ? 'w-3 h-3' : 'w-3.5 h-3.5'} ${s <= stars ? 'text-[#6366F1]' : 'text-gray-600/50'}`}
              fill="currentColor"
              viewBox="0 0 20 20"
              aria-hidden="true"
            >
  ...
  const renderEngagementIndicators = (draft: DraftWithMeeting) => {
    ...
    return (
      <div className="flex items-center gap-1">
        {(draft.openCount ?? 0) > 0 && (
          <span
            className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#6366F1]/20 text-[#6366F1]"
  ...
  return (
    <>
      <div id="drafts-table" className="bg-gray-900/60 border border-[#1E2A4A] light:bg-white light:border-gray-200 light:shadow-sm rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              {total > 0 && (
                <a
                  ...
                  download
                  className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-gray-400 hover:text-white bg-gray-700/50 hover:bg-gray-700 rounded-md transition-colors border border-gray-600/50 outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
        ...
        {/* Bulk Action Toolbar */}
        {selectedIds.size > 0 && (
          <div className="px-4 sm:px-6 py-3 border-b border-[#6366F1]/30 bg-[#6366F1]/10">
            {showBulkDeleteConfirm ? (
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-medium text-gray-300 bg-gray-700 border border-gray-600 rounded-lg hover:bg-gray-600 disabled:opacity-50 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    disabled={bulkDeleting}
                    className="px-3 py-1.5 text-xs font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
                  >
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <span className="text-sm text-[#818CF8]">Send {unsentSelectedDrafts.length} email{unsentSelectedDrafts.length > 1 ? 's' : ''} to their meeting hosts?</span>
                <div className="flex items-center gap-2">
                  <button
                  ...
                  <button
                    ...
                    disabled={bulkSending}
                    className="px-3 py-1.5 text-xs font-medium text-white bg-[#4F46E5] rounded-lg hover:bg-[#3A4BDD] disabled:opacity-50 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
                  >
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-[#818CF8] font-medium">{selectedIds.size} selected</span>
                  <button
                    onClick={() => setSelectedIds(new Set())}
                    className="text-xs text-gray-400 hover:text-white transition-colors rounded outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
                  >
                    Clear
                  </button>
                </div>
                <div className="flex items-center gap-2">
                  {bulkProgress && (
                    <span className="text-xs text-[#818CF8] tabular-nums">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-medium text-red-400 bg-red-500/10 border border-red-500/30 rounded-lg hover:bg-red-500/20 disabled:opacity-50 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
                  >
                    Delete
                  </button>
                  <button
                ...
                {draft.status !== 'sent' && (
                  ...
                  >
                    <input
                      ...
                      className="w-4 h-4 rounded border-gray-500 bg-gray-700 text-[#6366F1] focus:ring-[#6366F1] focus:ring-offset-0 cursor-pointer"
                    />
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="text-sm font-medium text-white light:text-gray-900 truncate">
                      ...
                      {draft.isDemo && (
                        <span className="ml-1.5 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#6366F1]/20 text-[#6366F1] border border-[#6366F1]/30">
                          Sample
                        </span>
                      )}
                    </h3>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="
                    ...
                    bg-[#6366F1]/10 light:bg-[#EEF0FF]
                    hover:bg-[#4F46E5]/20 light:hover:bg-[#EEF0FF]
                    active:scale-95
        ...
        <div className="hidden md:block">
          <table className="w-full table-fixed divide-y divide-[#6366F1]/10 light:divide-gray-200">
            <thead>
                ...
                <th className="w-[40px] px-3 py-4">
                  <label className="flex items-center">
                    <input
                      ...
                      className="w-4 h-4 rounded border-[#6366F1]/30 bg-gray-800/50 text-[#6366F1] focus:ring-[#6366F1] focus:ring-offset-0 cursor-pointer"
                    />
                ...
                return (
                  <Fragment key={draft.id}>
                    <tr
                      ...
                      className={`
                        group/row
                        ${selectedIds.has(draft.id) ? 'bg-[#6366F1]/10' : isExpanded ? 'bg-[#6366F1]/[0.06] light:bg-[#EEF0FF] border-l-2 border-l-[#F5F6FF]0' : 'hover:bg-white/[0.03] light:hover:bg-[#F5F6FF] border-l-2 border-l-transparent'}
                        cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#6366F1]
                    ...
                    >
                        ...
                        <div className="flex items-center gap-1.5">
                          {draft.qualityScore !== null ? (
                            <span className={`text-xs font-medium tabular-nums ${
                              draft.qualityScore >= 80 ? 'text-[#6366F1]' :
                              draft.qualityScore >= 60 ? 'text-[#818CF8]' : 'text-red-400'
                            }`}>
                      ...
                      <td className="px-2 py-3 whitespace-nowrap text-sm font-medium">
                        <button
                          ...
                          className="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-700/50 transition-all outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
        ...
        {totalPages > 1 && (
          <div className="px-5 sm:px-6 py-4 border-t border-[#6366F1]/10 light:border-gray-200">
            <div className="flex items-center justify-between">
