    ...
    <div className="bg-gray-900/50 light:bg-white border border-gray-700 light:border-gray-200 rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-8 h-8 rounded-lg bg-[#6366F1]/10 flex items-center justify-center">
          <svg className="w-4 h-4 text-[#6366F1]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-4"/></svg>
        </div>
          ...
          <div className="h-2.5 bg-gray-800 light:bg-gray-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-[width] duration-700 ${
                isAtLimit ? 'bg-red-500' : isWarning ? 'bg-amber-500' : 'bg-[#6366F1]'
              }`}
              style={{ width: `${pct}%` }}
            />
          </div>
      ...
      {tier === 'free' && isWarning && (
        <button
          ...
          className="mt-4 w-full flex items-center justify-between p-3 bg-[#6366F1]/5 border border-[#6366F1]/20 rounded-xl hover:bg-[#4F46E5]/10 transition-colors group outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060B18]"
        >
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-[#6366F1]" />
            <span className="text-sm text-[#6366F1] font-medium">Upgrade to Pro for unlimited drafts</span>
          </div>
          <span className="text-xs text-[#6366F1]/60 group-hover:text-[#6366F1] transition-colors">
            $19/mo &rarr;
          </span>
        </button>
      )}
