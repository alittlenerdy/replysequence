...
export function DashboardStats({ stats }: DashboardStatsProps) {
  ...
  const statCards = [
    ...
    },
    {
      ...
      textGradient: 'from-emerald-300 via-emerald-200 to-teal-300',
    ...
    },
    {
      ...
      textGradient: 'from-violet-300 via-violet-200 to-purple-300',
        ...
        const cardContent = (
          <motion.div
            ...
            className={`
              ...
              bg-white/[0.03] light:bg-white/80
              backdrop-blur-xl
              rounded-2xl p-5
            ...
            whileHover={{
              boxShadow: `0 0 40px ${stat.glowColor}, 0 8px 32px rgba(0,0,0,0.3)`,
            }}
          >
            {/* Icon container */}
            <div className={`
              relative mb-3 inline-flex rounded-xl p-2.5
            ...
            <div
              className={`text-3xl md:text-4xl font-black mb-1 tabular-nums bg-gradient-to-r ${stat.textGradient} bg-clip-text text-transparent light-solid-text ${stat.lightColor}`}
            >
            ...
            {/* Corner accent glow */}
            <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${stat.gradient} opacity-[0.04] rounded-bl-full transition-opacity duration-300 group-hover:opacity-[0.08]`} />
          </motion.div>
        );

        // Wrap hero card with gradient border
        if (stat.hero) {
          return (
            <motion.div key={stat.label} variants={item} className="relative rounded-2xl bg-gradient-to-br from-amber-400/30 via-yellow-400/20 to-orange-400/30 p-px">
              <div className="group relative overflow-hidden bg-[#0c0e14] light:bg-white/80 backdrop-blur-xl rounded-2xl p-5 cursor-pointer transition-all duration-300 hover:bg-white/[0.03]"
                style={{ boxShadow: '0 0 0 rgba(0,0,0,0)' }}
              >
                {/* Icon container */}
                <div className={`relative mb-3 inline-flex rounded-xl p-2.5 bg-gradient-to-br ${stat.gradient} shadow-lg shadow-black/20 transition-transform duration-300 group-hover:scale-110`}>
                  <div className="relative text-white">{stat.icon}</div>
                </div>

                {/* Number */}
                <div className={`text-3xl md:text-4xl font-black mb-1 tabular-nums bg-gradient-to-r ${stat.textGradient} bg-clip-text text-transparent light-solid-text ${stat.lightColor}`}>
                  <CountUp end={stat.value} duration={1500} />
                </div>

                {/* Label */}
                <div className="text-sm text-gray-400 light:text-gray-500 group-hover:text-gray-300 transition-colors duration-300">
                  {stat.label}
                </div>

                {/* Corner accent */}
                <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${stat.gradient} opacity-[0.06] rounded-bl-full transition-opacity duration-300 group-hover:opacity-[0.12]`} />
              </div>
            </motion.div>
          );
        }
