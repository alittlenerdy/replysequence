...
export function MeetingSummaryCard({
  ...
  return (
    <div className={`bg-gradient-to-br from-[#6366F1]/10 to-[#6366F1]/10 border border-[#6366F1]/20 rounded-2xl ${padding} light:shadow-sm`}>
      <h2 className={`${headingSize} font-semibold text-white light:text-gray-900 ${compact ? 'mb-2' : 'mb-4'} flex items-center gap-2`}>
        <svg className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} text-[#6366F1]`} fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          ...
          <div>
            ...
            <div className="space-y-2">
              {keyTopics.map((item, i) => (
                <div
                  key={i}
                  className={`flex items-center gap-2 ${compact ? 'px-2 py-1.5' : 'px-3 py-2'} bg-[#6366F1]/10 rounded-lg border border-[#6366F1]/15`}
                >
                  <span className={`${bodySize} text-gray-200 light:text-gray-700`}>{item.topic}</span>
                  {item.duration && (
                    <span className="text-xs text-[#6366F1]/60 ml-auto shrink-0">{item.duration}</span>
                  )}
                </div>
              ))}
            </div>
